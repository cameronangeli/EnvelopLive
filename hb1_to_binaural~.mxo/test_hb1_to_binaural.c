...
static
void
float_to_16bit_int(float **in, short *out, int n_ch, int buf_length)
{
  int i;
...
static
void
synthesize_pulse(float *buf, int length, long sample_rate)
{
  static int time = 0;
  int i;
  long interval = sample_rate / 2;
  
  int n_harmonics = 5;
  int fundamental = 1000;
...
/* apply separate 154-tap FIR filters to each spherical harmonic (generated from Heller and Benjamin's Faust code) */
static
void 
...
int main(int argc, char **argv)
{
  ...
  if (argc < 3)
    {
      fprintf(stderr, "\nUsage: test_hb1_to_binaural output_pcm_filename length_in_samples [azi1 azi2 ...]\n"
	      "\n\toutput_pcm_filename is a 2-ch 16-bit PCM file @ 44.1 kHz.\n"
	      "\n\t'azi<n>' is an azimuth coordinate in the range [-1, 1].  Each coordinate is applied "
	      "\n\taccording to a linear spacing in time spanning length_in_samples.\n"
	      "\n\tThe contents of the file are generated by first synthesizing a mono "
	      "\n\tpulse train, encoding it to horizontal B-format, and then transcoding "
	      "\n\tto binaural using Heller and Benjamin's Faust implementation.\n\n"
	      );
      return 1;
    }
  ...
  length = atoi(argv[2]);
  {
	...
	azimuths[0] = 0;
      ...
      {
	for (i = 0; i < n_azimuths; i++)
	  {
	    azimuths[i] = M_PI * atof(argv[i+3]);
	  }
      }	
    ...
    {
      int count = 0;
      int interval = length / n_azimuths;
      int cur_azi = 0;
      while (count < length)
	{       
	  synthesize_pulse(in, buf_length, sample_rate);
	  if (count > cur_azi * interval)
	    {
	      cur_azi++;
	    }
	  encode_wxy(buf_length, azimuths[cur_azi], 0, in, wxy);
